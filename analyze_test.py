...
# Take the genres of each song in id order
    ...
    total = list(test_result.values())  # (N, 4) list
    total_mean = np.mean(total, axis=0)  # (4,)
    total_min = np.min(total, axis=0)  # (4,)
    total_max = np.max(total, axis=0)  # (4,)
    total_result = np.stack((total_mean, total_min, total_max), axis=-1)  # (4, 3)
    ...
    genre_err = np.stack((min_err, max_err), axis=0)  # 2, num_genres, 4
    genre_total = np.stack((genre_mean.T, genre_min.T, genre_max.T), axis=-1)  # 4, genre, 3
